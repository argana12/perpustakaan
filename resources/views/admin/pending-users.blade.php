<x-app-layout>
    ...
    <div class="py-8">
        <div class="max-w-6xl mx-auto px-4 space-y-6">
            ...
            {{-- Flash Messages --}}
            @foreach (['status', 'class_added', 'class_deleted', 'major_added', 'major_deleted'] as $key)
                @if (session($key))
            ...
            {{-- ====================================================
                 ...
                 ==================================================== --}}
            <div>
                <h3 class="text-base font-semibold text-gray-800 mb-3">👥 Antrian Pengguna Pending</h3>
                                    ...
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-6 py-4 text-sm">
                                            @if ($user->kelas && $user->jurusan)
                                                <span class="text-green-700 font-medium">{{ $user->kelas }} · {{ $user->jurusan }}</span>
                                            @else
                                        ...
                                        <td class="px-6 py-4">
                                            <div class="flex flex-wrap gap-2">
                                                ...
                                                <button type="button"
                                                    onclick="openStudentModal({{ $user->id }}, '{{ addslashes($user->name) }}', '{{ addslashes($user->kelas ?? '') }}', '{{ addslashes($user->jurusan ?? '') }}')"
                                                ...
                                                <form method="POST" action="{{ route('admin.users.generate.code', $user) }}"
                                                    onsubmit="return confirm('Generate kode GURU untuk {{ addslashes($user->name) }}?')">
                                                    @csrf
                                                ...
                                                {{-- Tombol PETUGAS --}}
                                                <form method="POST" action="{{ route('admin.users.generate.code', $user) }}"
                                                    onsubmit="return confirm('Generate kode PETUGAS untuk {{ addslashes($user->name) }}?')">
                                                    @csrf
                                                ...
                                                <form method="POST" action="{{ route('admin.users.reject', $user) }}"
                                                    onsubmit="return confirm('Tolak pendaftaran {{ addslashes($user->name) }}?')">
                                                    @csrf
                                                    <button type="submit"
                                                        class="bg-red-500 hover:bg-red-600 text-white text-xs px-3 py-2 rounded-lg transition">
                                                        ✗ Tolak
                                                    </button>
                                                </form>
                                            </div>
        ...
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-hidden">
            ...
            <form id="studentModalForm" method="POST" action="" class="p-6 space-y-4">
                ...
                <div>
                    ...
                    @if ($classes->isEmpty())
                        <div class="p-3 bg-yellow-50 border border-yellow-200 rounded-lg text-yellow-700 text-sm">
                            ⚠️ Belum ada kelas. Tambahkan kelas terlebih dahulu di panel di atas.
                        </div>
                ...
                <div>
                    ...
                    @if ($majors->isEmpty())
                        <div class="p-3 bg-yellow-50 border border-yellow-200 rounded-lg text-yellow-700 text-sm">
                            ⚠️ Belum ada jurusan. Tambahkan jurusan terlebih dahulu di panel di atas.
                        </div>
    ...
    <script>
        function openStudentModal(userId, name, kelas, jurusan) {
            ...
            form.action = `/admin/pending-users/${userId}/generate-code`;
